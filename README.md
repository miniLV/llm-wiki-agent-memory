...
在 Codex 中打开此仓库并让 Agent 执行安装。Agent 会先检查环境，再安装资源、应用 `engineering-memory-loader`，并通过 Codex App 的正式工具创建或更新 Daily / Weekly automations。下面的配置界面保留为手动检查和修复入口。
...
4. 在 **自动化** 页复制 prompt 到 Codex App；该 prompt 会调用同一个 `agent-memory-setup` workflow，创建或更新 daily / weekly memory loop。
