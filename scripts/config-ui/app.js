...
const i18n = {
  zh: {
    ...
    "button.copyInstallRecovery": "复制恢复提示词并打开 Codex",
    "button.copyLog": "复制日志",
    ...
    "flow.sourceBody": "原始会话日志是完整事实源；daily loop 生成一份 bounded Evidence Snapshot，并把同一份内容一次性交给 agent。",
    "flow.dailyTitle": "02 Daily Wiki",
    "flow.dailyBody": "给人和 agent 看的每日中文 wiki；每个关键会话精确链接到 Snapshot Evidence Card。",
    "flow.conceptsTitle": "03 Reviewed Concepts",
    ...
    "automation.notice": "完整安装默认由 Codex Agent 非交互完成。这里用于检查 daily / weekly Memory Loop；下面的复制提示词仅是自动安装不可用时的恢复入口。",
    ...
    "loop.body": "正常安装请直接让当前 Codex Agent 执行完整设置。此操作只用于恢复：复制一段已明确授权完整安装的提示词并打开 Codex。",
    ...
    "toast.codexAutomationCopied": "恢复提示词已复制。仅在当前 Agent 无法使用自动化工具时粘贴发送。",
    ...
    "action.runner.manual.title": "恢复自动化安装",
    "action.runner.manual.body": "仅在 Agent 自动安装不可用时，复制恢复提示词并打开 Codex。",
    "action.runner.manual.label": "复制恢复提示词",
    "action.runner.auto.title": "恢复自动化安装",
    "action.runner.auto.label": "复制恢复提示词",
  ...
  en: {
    ...
    "button.copyInstallRecovery": "Copy Recovery Prompt & Open Codex",
    ...
    "flow.sourceBody": "Raw session logs remain the complete fact source; the daily loop creates one bounded Evidence Snapshot and sends those same bytes to the agent once.",
    "flow.dailyBody": "Daily Chinese wiki pages for humans and agents; every key topic links its exact Snapshot Evidence Cards.",
    ...
    "automation.notice": "Codex Agent normally performs the complete non-interactive installation. Use this page to inspect the daily / weekly Memory Loop; the copied prompt below is only a recovery path.",
    ...
    "loop.body": "For normal installation, ask the current Codex Agent to perform the complete setup. This recovery action copies an explicitly authorized setup prompt and opens Codex.",
    ...
    "toast.codexAutomationCopied": "Copied the recovery prompt. Paste it only when the current Agent cannot use automation tools.",
    ...
    "action.runner.manual.title": "Recover automation setup",
    "action.runner.manual.body": "Only when Agent installation is unavailable, copy the recovery prompt and open Codex.",
    "action.runner.manual.label": "Copy recovery prompt",
    "action.runner.auto.title": "Recover automation setup",
    "action.runner.auto.label": "Copy recovery prompt",
...
const setupActionKeys = {
  ...
  runner: {
    ...
    manual: {
      title: "恢复自动化安装",
      body: "仅在 Agent 自动安装不可用时，复制恢复提示词并打开 Codex。",
      label: "复制恢复提示词",
